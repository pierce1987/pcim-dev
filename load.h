...
private:
	...
	void save(Ar &ar, const unsigned int ver) const {
		ar & BOOST_SERIALIZATION_NVP(svarnames)
			& BOOST_SERIALIZATION_NVP(sisdiscrete)
			& BOOST_SERIALIZATION_NVP(snorm)
			& BOOST_SERIALIZATION_NVP(dvarnames)
			& BOOST_SERIALIZATION_NVP(dnorm);
	}
	template<typename Ar>
	void load(Ar &ar, const unsigned int ver) {
		ar & BOOST_SERIALIZATION_NVP(svarnames)
			...
			& BOOST_SERIALIZATION_NVP(dnorm);
		svarid.clear();
		for(int i=0;i<svarnames.size();i++)
			svarid[svarnames[i]] = i;
		dvarid.clear();
		for(int i=0;i<dvarnames.size();i++)
			dvarid[dvarnames[i]] = i;
	}
