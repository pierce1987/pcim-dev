...
public:
	...
	// new version, which is efficient in C++11 due to move semantics
	std::vector<int> ConsistentIndexes(const Instantiation &v) const;
	...
	std::vector<int> InconsistentIndexes(const Instantiation &v) const;
	...
	inline iimap::const_iterator begin() const { return card.begin(); }
	inline iimap::const_iterator end() const { return card.end(); }

	class idcontainer {
	private:
		const Context &c;
	public:
		inline idcontainer(const Context &con) : c(con) {}
		struct getfirst : std::unary_function<std::pair<int,int>,int> {
			inline int operator()(const std::pair<int,int> &p) const { return p.first; }
		};
		using iterator=boost::transform_iterator<getfirst,iimap::const_iterator>;
		iterator begin() const { return iterator(c.begin()); }
		iterator end() const { return iterator(c.end()); }
	};

	// use this to get a container of just the variable ids
	inline idcontainer Vars() const { return idcontainer(*this); }

protected:
	iimap card; // maps (id) to (number of values)
	int size;
...
public:
	class iterator {
	public:
		iterator(const Instantiation &si, bool done=false)
			: i(si), d(done) {}
		iterator(Instantiation &&si, bool done=false)
			: i(std::move(si)), d(done) {}
		iterator &operator++() { d = !i.Inc(); return *this; }
		iterator operator++(int){ iterator ret(*this); ++(*this); return ret;}
		Instantiation &operator*() { return i; }
		Instantiation *operator->() { return &i; }
		const Instantiation &operator*() const { return i; }
		const Instantiation *operator->() const { return &i; }
		bool operator==(const iterator &ii) const
			{ return (ii.d&&d) || (ii.d==d && ii.i==i); }
		bool operator!=(const iterator &ii) const { return !(*this==ii); }
		bool isdone() const { return d; }
	protected:
		Instantiation i;
		bool d;
	};
...
public:
	...
	public:
		...
		iterator(Instantiation &&si, const Context &incc, bool done=false)
			: InstantRange::iterator(std::move(si),done), c(incc) {}
...
public:
	...
	public:
		iterator(const Instantiation &si, const Context &c,
			...
			incsame = includesame;
			if (!done) {
				vars = c.VarList(); // TODO:  remove and replace
				chvar = 0;
				nextvar();
			}
		}
		iterator(Instantiation &&si, const Context &c,
				bool includesame=false,bool done=false)
				: InstantRange::iterator(std::move(si),done) {
			incsame = includesame;
			if (!done) {
				vars = c.VarList(); // TODO:  remove and replace
				chvar = 0;
...
// for C++11, we can make helper functions that allow use of range-based for
// (for C++03, see macros below)

} // temp end ctbn namespace

// allows range-based for directly on a Context:
inline ctbn::InstantRange::iterator begin(const ctbn::Context &c)
	{ return ctbn::InstantRange::iterator(ctbn::Instantiation(c,0)); }
inline ctbn::InstantRange::iterator end(const ctbn::Context &c)
	{ return ctbn::InstantRange::iterator(ctbn::Instantiation(),true); }

namespace ctbn {

// syntax to place around context to clarify what range-based for is over:

// range of all assignments to c
inline InstantRange assign(const Context &c) { return InstantRange(c); }
// range of all assignments to c, plus additional assignments as in i
inline InstantSubRange assign_with(const Context &c, const Instantiation &i)
	{ return InstantSubRange(i,c); }
// range over all assignments to i consistent with i (ie only over unassigned
//   portions of i)
inline InstantSubRange completion(const Instantiation &i)
	{ return InstantSubRange(i,i.MissingVars()); }
// range over all of c not assigned in i
// (same as assign_with, except that if variable is in c and i we *do*
//  change it here, but *not* in assign_with)
inline InstantSubRange assign_except(const Context &c, const Instantiation &i)
	{ return InstantSubRange(i,c-i); }

// some macros for C++03
// 
...
// range over all assignments that differ by 1 variable from i
inline DiffByOneRange assign_1diff(const Instantiation &i)
	{ return DiffByOneRange(i,i,false); }
// range over all assignments that differ by 0 or 1 variable from i
inline DiffByOneRange assign_01diff(const Instantiation &i)
	{ return DiffByOneRange(i,i,true); }
// same as above, but only change variables in c
inline DiffByOneRange assign_1diff(const Context &c, const Instantiation &i)
	{ return DiffByOneRange(i,c,false); }
inline DiffByOneRange assign_01diff(const Context &c, const Instantiation &i)
	{ return DiffByOneRange(i,c,true); }
